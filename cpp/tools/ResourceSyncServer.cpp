...
void ResourceSyncServer::ClientConnection::HandleResourceUpdated(const json& message) {
    ...
    // 提取消息内容
    std::string resourceName = message.value("resource_name", "");
    std::string resourcePath = message.value("resource_path", "");
    std::string resourceTypeStr = message.value("resource_type", "TEXTURE");
    uint32_t assetId = message.value("asset_id", 0u);
    
    std::cout << "  资源名称: " << resourceName << std::endl;
    std::cout << "  资源路径: " << resourcePath << std::endl;
    std::cout << "  资源类型: " << resourceTypeStr << std::endl;
    std::cout << "  资源ID: " << assetId << std::endl;
    
    // 1. 检查资源文件是否存在
    if (resourcePath.empty() || resourceName.empty()) {
        std::cerr << "错误: 资源路径或名称为空" << std::endl;
        return;
    }
    
    std::ifstream resourceFile(resourcePath);
    if (!resourceFile.good()) {
        std::cerr << "错误: 资源文件不存在: " << resourcePath << std::endl;
        return;
    }
    resourceFile.close();
    ...
    if (resourceTypeStr == "TEXTURE" || resourceTypeStr == "TEXTURE_2D") {
        resourceType = ResourceType::TEXTURE_2D;
    } else if (resourceTypeStr == "TEXTURE_CUBE") {
        resourceType = ResourceType::TEXTURE_CUBE;
    } else if (resourceTypeStr == "MODEL") {
        resourceType = ResourceType::MODEL;
    } else if (resourceTypeStr == "MATERIAL") {
        resourceType = ResourceType::MATERIAL;
    } else if (resourceTypeStr == "SHADER") {
        resourceType = ResourceType::SHADER;
    } else if (resourceTypeStr == "SOUND") {
        resourceType = ResourceType::SOUND;
    } else if (resourceTypeStr == "ANIMATION") {
        resourceType = ResourceType::ANIMATION;
    } else if (resourceTypeStr == "PARTICLE_SYSTEM") {
        resourceType = ResourceType::PARTICLE_SYSTEM;
    } else if (resourceTypeStr == "SCRIPT") {
        resourceType = ResourceType::SCRIPT;
    }
    
    // 3. 更新资源包中的资源 (TODO: 实现完整逻辑)
    // 这里简单处理：如果资源ID有效，重新加载资源
    if (assetId > 0) {
        std::cout << "  重新加载资源: " << assetId << std::endl;
        bool success = ResourceManager::GetInstance().ReloadResource(assetId);
        std::cout << "  重载结果: " << (success ? "成功" : "失败") << std::endl;
        
        // 4. 通知资源管理器资源已更新
        if (success) {
            std::cout << "  资源已成功更新并通知资源管理器" << std::endl;
        }
    } else {
        // 新资源，需要分配资源ID并加载
        std::cout << "  加载新资源: " << resourceName << std::endl;
        AssetID newAssetId = ResourceManager::GetInstance().LoadResource(resourceName, resourceType);
        if (newAssetId > 0) {
            std::cout << "  新资源加载成功，ID: " << newAssetId << std::endl;
        } else {
            std::cerr << "  新资源加载失败" << std::endl;
        }
    }
}
...
void ResourceSyncServer::ClientConnection::HandleAssetIDsHeader(const json& message) {
    ...
    // 提取头文件内容和路径
    std::string headerContent = message.value("content", "");
